...
public class GuiBossOverlay extends Gui {
   ...
   public void renderBossHealth() {
      if (!this.mapBossInfos.isEmpty()) {
         ...
         for(BossInfoLerping bossinfolerping : this.mapBossInfos.values()) {
            int k = i / 2 - 91;
            BossInfo event = ForgeHooksClient.bossBarRenderPre(scaledresolution, bossinfolerping, k, j, 10 + this.client.fontRendererObj.FONT_HEIGHT);
            if (!event.isCanceled()) {
   ...
   public boolean shouldPlayEndBossMusic() {
      if (!this.mapBossInfos.isEmpty()) {
         for(net.minecraft.world.BossInfo bossinfo : this.mapBossInfos.values()) {
