...
public class GameSettings {
   ...
   public void loadOptions() {
      try {
         ...
         List list = IOUtils.readLines(new FileInputStream(this.optionsFile));
         NBTTagCompound nbttagcompound = new NBTTagCompound();

         for(String s : list) {
            try {
               Iterator iterator = COLON_SPLITTER.omitEmptyStrings().limit(2).split(s).iterator();
      ...
      if (!FMLClientHandler.instance().isLoading()) {
         ...
         try {
            ...
            for(KeyBinding keybinding : this.keyBindings) {
               ...
               printwriter.println(keybinding.getKeyModifier() != KeyModifier.NONE ? keyString + ":" + keybinding.getKeyModifier() : keyString);
            }
