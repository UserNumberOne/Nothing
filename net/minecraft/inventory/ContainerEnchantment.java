            ...
            if (!this.worldPointer.isRemote) {
               ...
               for(int j = -1; j <= 1; ++j) {
                  for(int k = -1; k <= 1; ++k) {
                     if ((j != 0 || k != 0) && this.worldPointer.isAirBlock(this.position.add(k, 0, j)) && this.worldPointer.isAirBlock(this.position.add(k, 1, j))) {
                        power = power + ForgeHooks.getEnchantPower(this.worldPointer, this.position.add(k * 2, 0, j * 2));
                        power = power + ForgeHooks.getEnchantPower(this.worldPointer, this.position.add(k * 2, 1, j * 2));
                        if (k != 0 && j != 0) {
                           power = power + ForgeHooks.getEnchantPower(this.worldPointer, this.position.add(k * 2, 0, j));
                           power = power + ForgeHooks.getEnchantPower(this.worldPointer, this.position.add(k * 2, 1, j));
                           power = power + ForgeHooks.getEnchantPower(this.worldPointer, this.position.add(k, 0, j * 2));
                           power = power + ForgeHooks.getEnchantPower(this.worldPointer, this.position.add(k, 1, j * 2));
                        }
                     }
                  }
               }
               ...
               for(int j1 = 0; j1 < 3; ++j1) {
                  if (this.enchantLevels[j1] > 0) {
                     List list = this.getEnchantmentList(itemstack, j1, this.enchantLevels[j1]);
